...
\caption{我们设计的三个实验的流程，左到右分别代表图像重构、新视角生成和材质迁移}
\label{fig:sample}
\end{figure}
...
本节将讨论我们的方法的具体实现，包括各个模型的输入输出、参数设置和训练过程。上一节的讨论可以得到我们需要实现的映射包括：从真实图片中提取内容和材质隐变量，重构深度图和RGB图片
...
\midrule
baseline(0203) & 0.671 & 0504跑结果 & 0504跑结果 \\
...
下面我们展示我们的模型在真实图片上的结果。在合成数据集上训练的模型，不能有效的提取出真实图片中的材质信息，因此我们用真实图片的一部分精修训练好的模型，再在测试集上面测试，精修后的模型能够将真实图片和生成的训练图片的材质信息映射到统一分布上，见图7。图8、图9分别展示了将真实图片的材质信息应用到不同的三维模型和在真实图片的材质间插值的结果。同时，应用视角估计方法的结果，我们可以将真实图片的视角和材质都迁移到深度图上，结果如图10。
